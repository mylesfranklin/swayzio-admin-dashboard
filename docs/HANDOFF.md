...
| Swayzio OS | Neon project `swayzio-os` (`sparkling-butterfly-49751147`). ELT every 6h via GitHub Actions. Migrations applied through **0013**. Semantic recall live (41 rows embedded). |
| Eve agent | `/agent` in the dashboard. eve **0.19.0 (pinned exact)**, Sonnet 5 via AI Gateway. 13 read tools (RO Postgres role) + approval-gated `trigger_sync`. Clerk "eve" JWT template (dev + prod instances) carries `aud`/`email`/`role`. |
