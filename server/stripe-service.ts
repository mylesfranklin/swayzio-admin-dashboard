...
export class StripeService {
  ...
  /**
   * Count subscriptions canceled in the last `days` days using the Events API.
   * Events are ordered by the event's own created time (= cancellation time), so this
   * is not limited by subscription creation date and captures ALL recent cancellations —
   * including subs created years ago that were canceled recently. Stripe retains events
   * for 30 days in live mode, which exactly covers the 30-day churn window.
   */
  async getRecentCancellationCount(days: number = 30): Promise<number> {
    const stripe = getUncachableStripeClient();
    const sinceTs = Math.floor(Date.now() / 1000) - days * 24 * 60 * 60;
    let count = 0;
    let startingAfter: string | undefined;
    do {
      const params: any = { type: 'customer.subscription.deleted', created: { gte: sinceTs }, limit: 100 };
      if (startingAfter) params.starting_after = startingAfter;
    ...
    console.log(`Recent cancellation count (${days}d, from events): ${count}`);
    return count;
  }
  ...
  async getDashboardStats(): Promise<{
    ...
    try {
      ...
      const [nonCanceled, recentlyCanceled, canceledLast30DaysCount] = await Promise.all([
        this.getNonCanceledSubscriptions(),                    // non-canceled subs for MRR + active counts
        this.getAllSubscriptions({ status: 'canceled', maxPages: 60 }), // canceled subs for 12-month trend reconstruction
        this.getRecentCancellationCount(30)                   // events-based: accurate 30-day churn (not limited by creation order)
      ]);
      ...
      // MRR is scoped to USD-denominated subscriptions only to avoid mixing incompatible
      // currency amounts into a single dollar figure. Non-USD subs are counted and logged
      // but excluded from the MRR sum so the reported number is mathematically valid.
      const activeSubsUSD = activeSubscriptions.filter(s => s.currency === 'usd');
      const activeSubsNonUSD = activeSubscriptions.filter(s => s.currency !== 'usd');
      const mrrCents = activeSubsUSD.reduce((sum, sub) => sum + sub.monthlyAmount, 0);
      ...
      console.log(`MRR (USD only): $${(mrrCents / 100).toFixed(2)} from ${activeSubsUSD.length} active USD subs`);
      if (activeSubsNonUSD.length > 0) {
        console.log(`  (excluded from MRR) ${activeSubsNonUSD.length} non-USD active subs: ${JSON.stringify(currencyBreakdown)}`);
      }
