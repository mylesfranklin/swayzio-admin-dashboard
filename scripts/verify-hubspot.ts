...
const { getContactCounts, getProDistribution, getContactGrowth, getPowerUsers, getCatalogScan, getReacquireCandidates, getEnumDistribution } =
  await import("../src/server/integrations/hubspot.ts");
const { getOrCompute } = await import("../src/server/cache.ts");
const MIN = 60_000;
const t0 = Date.now();
const [counts, pro, growth, power, catalog, reacquire, acq, roles, ctypes] = await Promise.all([
...
console.log("\nReacquire candidates:", reacquire.data.totalTargets.toLocaleString(), "| emails:", reacquire.data.emails.length);
console.log("  by month:", reacquire.data.byMonth.map((m: { month: string; candidates: number; highValue: number }) => `${m.month}:${m.candidates}/${m.highValue}`).join("  "));
